...
MainWindow::MainWindow(QDir const& temp_directory, bool multiple,
  ...
  m_messageClient {new MessageClient {QApplication::applicationName (),
        ...
        this}},
  m_psk_Reporter {&m_config, QString {"WSJT-X v" + version () + " i+ JP1LRT"}.simplified ()},     // UR
  ...
  m_useDarkStyle {false}
{
  ...
  createStatusBar();
  m_syncDiagLogPath = QCoreApplication::applicationDirPath() + "/sync_diag_log.csv";
  ...
  ui->decodedTextBrowser2->set_mycall_full_width_highlight (true);
  ...
  ui->respondComboBox->addItem("CQ: AutoSeq 2");
  ui->respondComboBox->addItem("CQ: AutoSeq 3");
  ...
  m_manualNextTargetLocked=false;
  m_bCallingCQ=false;
...
// Get power, spectrum, and ihsym
  ...
  // AutoSeq 2: TX開始前（hsymStop）に最優先局を選択
  if(m_ihsym==m_hsymStop
      && ui->respondComboBox->isVisible()
      && ui->respondComboBox->currentText()=="CQ: AutoSeq 2"
      && !m_autoSeqCandidates.isEmpty()) {
    auto best = bestAutoSeqCandidate();
    if (best.category != ReplyPriorityCategory::None && !best.decoded_line.isEmpty()) {
      DecodedText dt {best.decoded_line};
    ...
    m_autoSeqCandidates.clear();
    m_autoSeqDecodeOrder = 0;
  }
  if(bCallDecoder) {
    if(m_mode=="Echo") {
      float dBerr=0.0;
  ...
  if(bmsk144 and (line[0]!=0)) {
    ...
    if (!filtered or m_config.filters_for_Wait_and_Pounce_only()) {
        ...
        ui->decodedTextBrowser->displayDecodedText (decodedtext, m_config.my_callsign (), m_mode, m_config.DXCC (),
          ...
          haveFSpread, fSpread, bDisplayPoints, m_points, distance, m_muted, m_hisCall);
        if(m_position != 0) ui->decodedTextBrowser->horizontalScrollBar()->setValue(m_position);
        ...
        if (m_mode=="MSK144" && text.mid(22).contains(m_baseCall + " " + m_hisCall + " 73")) {
            ui->decodedTextBrowser2->displayDecodedText (decodedtext, m_config.my_callsign (), m_mode, m_config.DXCC (),
              m_logBook, m_currentBand, m_config.ppfx (), false, false, 0.0, false, -99, "", m_muted, m_hisCall);
        }
    }
...
void MainWindow::createStatusBar()                           //createStatusBar
{
  ...
  ave_label.setAlignment (Qt::AlignHCenter);
  ave_label.setMinimumSize (QSize {70, 18});
  ave_label.setFrameStyle (QFrame::Panel | QFrame::Sunken);
  statusBar()->addWidget (&ave_label);

  lag_label.setAlignment (Qt::AlignHCenter);
  lag_label.setMinimumSize (QSize {95, 18});
  lag_label.setFrameStyle (QFrame::Panel | QFrame::Sunken);
  statusBar()->addWidget (&lag_label);

  sync_toggle_button.setText("Sync OFF");
  sync_toggle_button.setCheckable(true);
  sync_toggle_button.setChecked(false);
  sync_toggle_button.setMinimumSize(QSize {60, 18});
  statusBar()->addWidget(&sync_toggle_button);
  sync_toggle_button.setStyleSheet("QPushButton:checked { background-color: #00ff00; color: #000000; border-style: outset; border-width: 1px; border-radius: 5px; border-color: black; padding: 3px; }");
  connect(&sync_toggle_button, &QPushButton::toggled, [this](bool checked) {
    bool wasOff = !m_syncWasEnabled;
    m_useDtOffset = checked;
    sync_toggle_button.setText(checked ? "Sync ON" : "Sync OFF");
    // OFF→ON遷移時のみ一発ジャンプ
    if (checked && wasOff && m_lastJumpReady) {
      double jumpVal = m_lastJumpAvgDt;
      if (jumpVal > 0.5) jumpVal = 0.5;
      if (jumpVal < -0.5) jumpVal = -0.5;
      if (std::fabs(jumpVal - m_dtOffset) >= m_jumpThreshold) {
        m_dtOffset = jumpVal;
      }
    }
    m_syncWasEnabled = checked;
  });
...
//Left (Band activity) window
    ...
    if(!m_bFastDone) {
      ui->decodedTextBrowser->displayDecodedText (decodedtext, m_config.my_callsign (), m_mode, m_config.DXCC (),
         m_logBook, m_currentBandPeriod, m_config.ppfx (), false, false, 0.0, false, -99, "", m_muted, m_hisCall);
      if(m_position != 0) ui->decodedTextBrowser->horizontalScrollBar()->setValue(m_position);
    }
...
void MainWindow::decodeDone ()
{
  ...
  double tdone = fmod(tnow.time().msecsSinceStartOfDay() / 1000.0, m_TRperiod);
  int mswait;
  ...
  bool isIntermediatePass = (m_mode == "FT8") && m_multithreadFT8 && (m_hsymStop != dec_data.params.nzhsym);

  if (!isIntermediatePass && (m_mode == "FT8" || m_mode == "FT4")) {
    double lag = tdone;
    if (lag > 0.5 * m_TRperiod) {
      lag -= m_TRperiod;
    }
    double lag_for_candidate = 999.0;
    bool lag_ok_for_candidate = false;
    if (std::abs(lag) > 2.0) {
      lag_label.setText("Lag=---/" + QString::number(m_nDecodes));
    } else {
      lag_for_candidate = lag;
      lag_ok_for_candidate = true;
      QString sLag;
      sLag.setNum(lag, 'f', 2);
      if (lag > 0.0) sLag = "+" + sLag;
      lag_label.setText("Lag=" + sLag + "/" + QString::number(m_nDecodes));
    }

    if (!m_dtValues.isEmpty()) {
      float avg = 0.0f;
      int used = 0;
      float spread = 0.0f;
      bool valid = false;
      if (evaluateAverageDt(m_dtValues, avg, used, spread, valid)) {
        m_lastAvgSpread = spread;
        m_lastAvgUsed = used;
        m_lastAvgValid = valid;
        if (valid) {
          ++m_avgValidStreak;
          m_lastValidAvgDt = avg;
        } else {
          m_avgValidStreak = 0;
        }
        m_syncCandidate = valid
                          && (m_avgValidStreak >= 3)
                          && lag_ok_for_candidate
                          && (std::fabs(lag_for_candidate) <= 0.25);
        bool offset_update_ok =
            m_syncCandidate
            && (std::fabs(m_lastValidAvgDt - m_dtOffset) <= 0.30)
            && (m_lastAvgUsed >= 6);
        m_offsetUpdateAccepted = offset_update_ok;
        if (offset_update_ok) {
          m_nonCandidateStreak = 0;
          double target = 0.8 * m_dtOffset + 0.2 * m_lastValidAvgDt;
          double delta = target - m_dtOffset;
          if (delta > 0.05) delta = 0.05;
          if (delta < -0.05) delta = -0.05;
          m_dtOffset += delta;
          m_lastOffsetDelta = delta;
          if (m_dtOffset > 1.0) m_dtOffset = 1.0;
          if (m_dtOffset < -1.0) m_dtOffset = -1.0;
        } else {
          if (!m_syncCandidate) {
            ++m_nonCandidateStreak;
            if (m_nonCandidateStreak >= 3) {
              m_dtOffset *= 0.90;
              if (std::fabs(m_dtOffset) < 0.01) m_dtOffset = 0.0;
            }
          }
          m_lastOffsetDelta = 0.0;
        }
        QString sOfs;
        sOfs.setNum(m_dtOffset, 'f', 2);
        if (m_dtOffset >= 0.0) sOfs = "+" + sOfs;
        QString sAvg;
        sAvg.setNum(avg, 'f', 1);
        if (avg > 0.0f) sAvg = "+" + sAvg;
        QString label = "Avg=" + sAvg + "/" + QString::number(used);
        if (valid) {
          label += " [" + QString::number(m_avgValidStreak) + "]";
          if (m_syncCandidate) {
            label += " C";
            if (m_offsetUpdateAccepted) label += " U";
          }
        } else {
          label += "*";
        }
        label += " O=" + sOfs;
        ave_label.setText(label);
        writeSyncDiagLog(avg, used, spread, valid, m_syncCandidate, m_offsetUpdateAccepted, lag_for_candidate);
        // Jump snapshot: 毎周期更新
        {
          double jAvg = 0.0; int jUsed = 0; float jSpread = 0.0f;
          if (computeJumpAverageDt(m_dtValues, jAvg, jUsed, jSpread)) {
            m_lastJumpAvgDt = jAvg;
            m_lastJumpUsed  = jUsed;
            m_lastJumpSpread = jSpread;
            m_lastJumpReady = valid && m_syncCandidate
                              && (jUsed >= 6)
                              && (jSpread <= 0.5f);
          } else {
            m_lastJumpReady = false;
          }
        }
      } else {
        m_lastAvgSpread = 0.0f;
        m_lastAvgUsed = 0;
        m_lastAvgValid = false;
        m_avgValidStreak = 0;
        m_syncCandidate = false;
        ave_label.setText("Avg=");
      }
    } else {
      ave_label.setText("Avg=");
    }
  ...
  // AutoSeq 3: 最終decodeDone時のみ選局
  {
    bool isFinalDecode = (m_mode != "FT8")
        || (dec_data.params.nzhsym == m_hsymStop)
        || (m_multithreadFT8 && m_hsymStop == dec_data.params.nzhsym);
    if(isFinalDecode
        && ui->respondComboBox->isVisible()
        && ui->respondComboBox->currentText()=="CQ: AutoSeq 3"
        && !m_autoSeqCandidates.isEmpty()) {
      auto best = bestAutoSeqCandidate();
      if (best.category != ReplyPriorityCategory::None && !best.decoded_line.isEmpty()) {
        DecodedText dt {best.decoded_line};
...
void MainWindow::readFromStdout()                             //readFromStdout
{
  ...
  // Filtering out some false decodes, and don't write all.txt for such
    ...
    if ((m_mode=="FT8" || m_mode=="FT4") && !decodedtext.isTX()
        && line_read.indexOf("<DecodeFinished>") < 0) {
      m_dtValues.append(decodedtext.dt());
    }
    ...
    if(line_read.indexOf("<DecodeFinished>") >= 0) {
      ...
      if(m_mode=="Q65") {
        ...
        if(m_nDecodes==0 && !(m_multithreadFT8 && m_diskData)) ndecodes_label.setText("");
      }
      decodeDone ();
      return;
    } else {
      m_nDecodes+=1;
      if(m_mode!="Q65") ndecodes_label.setText("");
...
//Left (Band activity) window
      if(!bAvgMsg) {
        ...
        // CQ: AutoSeq 2/3 - 候補局を積む
        {
          const QString modeText = ui->respondComboBox->currentText();
          const bool autoSeqMode = ui->respondComboBox->isVisible()
              && (modeText == "CQ: AutoSeq 2" || modeText == "CQ: AutoSeq 3")
              && !filtered && !ignored
              && !(ui->actionFull_Duplex_Mode->isChecked() && m_txing);
          if (autoSeqMode) {
            QString norm = text;
            norm.replace("~", " ");
            norm = norm.simplified().toUpper();
            const QString myCall = m_config.my_callsign().trimmed().toUpper();
            QStringList tok = norm.split(QLatin1Char(' '), Qt::SkipEmptyParts);
            bool is73 = norm.contains(" RR73") || norm.endsWith("RR73")
                     || norm.contains(" 73") || norm.endsWith("73");
            bool isCurrentQso73 = false;
            if (tok.size() >= 3
                && tok[0] == myCall
                && Radio::base_callsign(tok[1]) == Radio::base_callsign(m_hisCall.trimmed().toUpper())
                && (tok[2] == "RR73" || tok[2] == "73")) {
              isCurrentQso73 = true;
            }
            QString replyCall;
            if (!is73 || isCurrentQso73) {
              for (int ii = 0; ii + 1 < tok.size(); ++ii) {
                if (tok[ii] == myCall) {
                  QString next = tok[ii+1];
                  if (next != "RR73" && next != "73" && next.contains(QRegularExpression("[0-9]")))
                    replyCall = next;
                  break;
                }
              }
            }
            if (!replyCall.isEmpty()) {
              AutoSeqCandidate cand;
              cand.callsign     = replyCall;
              cand.grid         = "";
              {
                QString dc, dg;
                decodedtext.deCallAndGrid(dc, dg);
                cand.grid = dg;
              }
              cand.snr          = decodedtext.snr();
              cand.decode_order = m_autoSeqDecodeOrder++;
              cand.decoded_line = text;
              cand.is_lotw      = m_config.lotw_users().user(replyCall);
              // 独立した相手局属性判定（CQPriority()に依存しない）
              cand.category = isCurrentQso73
                  ? ReplyPriorityCategory::WantedCallsign
                  : classifyStationPriority(replyCall, cand.grid);
              if (cand.grid.size() >= 4) {
                double utch=0.0; int nAz,nEl,nDmiles,nDkm,nHotAz,nHotABetter;
                azdist_(const_cast<char*>((m_config.my_grid()+"      ").left(6).toLatin1().constData()),
                        const_cast<char*>((cand.grid+"      ").left(6).toLatin1().constData()),
                        &utch,&nAz,&nEl,&nDmiles,&nDkm,&nHotAz,&nHotABetter,6,6);
                cand.distance_km = nDkm;
              }
              bool found = false;
              for (auto& existing : m_autoSeqCandidates) {
                if (existing.callsign == cand.callsign) {
                  if (isBetterCandidate(cand, existing)) existing = cand;
                  found = true; break;
                }
              }
              if (!found) m_autoSeqCandidates.append(cand);
            }
          }
        }
        ...
        if (!pounce && (m_config.highlight_DXcall() or m_config.alert_Enabled()) && (m_hisCall != "") &&
          ...
          if (m_config.highlight_DXcall()) {
            // repeated highlighting to override JTAlert
            ui->decodedTextBrowser->highlight_callsign(m_hisCall, QColor(173,216,230), QColor(0,0,0), true);
            QTimer::singleShot (500, [=] {ui->decodedTextBrowser->highlight_callsign(m_hisCall, QColor(173,216,230), QColor(0,0,0), true);});
            QTimer::singleShot (1000, [=] {ui->decodedTextBrowser->highlight_callsign(m_hisCall, QColor(173,216,230), QColor(0,0,0), true);});
            QTimer::singleShot (2500, [=] {ui->decodedTextBrowser->highlight_callsign(m_hisCall, QColor(173,216,230), QColor(0,0,0), true);});
          }
        }
...
void MainWindow::guiUpdate()
{
  ...
  qint64 ms_raw = QDateTime::currentMSecsSinceEpoch();
  qint64 ms = m_useDtOffset
            ? (ms_raw + qint64(m_dtOffset * 1000.0)) % 86400000
            : ms_raw % 86400000;
  if (ms < 0) ms += 86400000;
  int nsec=ms/1000;
...
//----------------------------------------------------------------------
    ...
    if(m_restart) {
      // Avoid duplicate Tx logging and duplicate Rx Frequency window insertion on restart path.
      // The actual Tx-start path below (g_iptt == 1 && m_iptt0 == 0) remains the primary path.
    }
...
//----------------------------------------------------------------------
  ...
  if (g_iptt == 1 && m_iptt0 == 0) {
    ...
    if (m_mode != "FST4W" && m_mode != "WSPR" && m_mode!="Echo")
      {
        if(!m_tune) write_all("Tx", current_message.trimmed());
...
//  }
  ...
  if(ui->txFirstCheckBox->isVisible() && !ui->txFirstCheckBox->isEnabled() && (
    ...
    m_bDoubleClicked = true;
    m_manualNextTargetLocked = true;
    m_manualNextTargetCall = Radio::base_callsign(m_hisCall);
...
void MainWindow::processMessage (DecodedText const& message, Qt::KeyboardModifiers modifiers)
{
  ...
  if (m_manualNextTargetLocked && !m_bDoubleClicked && Radio::base_callsign(hiscall) != m_manualNextTargetCall) return;
...
//      w34=w.at(nw-2);
  ...
  if (Radio::is_callsign (hiscall)
    ...
    if (m_bDoubleClicked || !m_manualNextTargetLocked) {
...
void MainWindow::cease_auto_Tx_after_QSO ()
{
  if (SpecOp::FOX != m_specOp
      && ui->cbAutoSeq->isVisible () && ui->cbAutoSeq->isEnabled () && ui->cbAutoSeq->isChecked ())
    {
      auto modeText = ui->respondComboBox->currentText();
      if (modeText == "CQ: AutoSeq 2" || modeText == "CQ: AutoSeq 3") {
        return;
      }
...
bool MainWindow::robustAverageDt(QVector<float> const& values, float& avg_out, int& used_out) const
{
  avg_out = 0.0f;
  used_out = 0;
  ...
  avg_out = sum / static_cast<float>(used_out);
  return true;
}

bool MainWindow::evaluateAverageDt(QVector<float> const& values, float& avg_out, int& used_out, float& spread_out, bool& valid_out) const
{
  avg_out = 0.0f;
  used_out = 0;
  spread_out = 0.0f;
  ...
  avg_out = sum / static_cast<float>(used_out);
  spread_out = v[end - 1] - v[start];

  valid_out = (used_out >= 5) && (spread_out <= 0.6f);

  return true;
}
bool MainWindow::computeJumpAverageDt(QVector<float> const& values, double& avg_out, int& used_out, float& spread_out) const
{
  avg_out = 0.0;
  used_out = 0;
  spread_out = 0.0f;
  ...
  used_out = end - start;
  if (used_out <= 0) return false;
  avg_out = static_cast<double>(sum) / used_out;
  spread_out = v[end - 1] - v[start];
  return true;
}

ReplyPriorityCategory MainWindow::classifyStationPriority(QString const& callsign, QString const& grid) const
{
    if (callsign.isEmpty()) return ReplyPriorityCategory::None;

    auto const& looked_up = m_logBook.countries()->lookup(callsign);

    // 全モード・全バンド判定（バンド引数なし）
    bool callB4 {false}, countryB4 {false}, gridB4 {false};
    bool contB4 {false}, cqzB4 {false}, ituzB4 {false};
    m_logBook.match(callsign, m_mode, grid, looked_up,
                    callB4, countryB4, gridB4, contB4, cqzB4, ituzB4);

    // 現バンド判定
    bool callB4b {false}, countryB4b {false}, gridB4b {false};
    bool contB4b {false}, cqzB4b {false}, ituzB4b {false};
    m_logBook.match(callsign, m_mode, grid, looked_up,
                    callB4b, countryB4b, gridB4b, contB4b, cqzB4b, ituzB4b,
                    m_currentBand);

    // 優先度判定（高い順）
    // 全バンド新規 > バンド新規 の順
    if (!countryB4)  return ReplyPriorityCategory::NewDXCC;
    if (!countryB4b) return ReplyPriorityCategory::NewDXCCBandMode;
    if (!cqzB4)      return ReplyPriorityCategory::NewCQZone;
    if (!cqzB4b)     return ReplyPriorityCategory::NewCQZoneBandMode;
    if (!ituzB4)     return ReplyPriorityCategory::NewITUZone;
    if (!ituzB4b)    return ReplyPriorityCategory::NewITUZoneBandMode;
    if (!gridB4)     return ReplyPriorityCategory::NewGrid;
    if (!gridB4b)    return ReplyPriorityCategory::NewGridBandMode;
    if (!contB4)     return ReplyPriorityCategory::NewContinent;
    if (!contB4b)    return ReplyPriorityCategory::NewContinentBandMode;
    if (!callB4)     return ReplyPriorityCategory::NewCallsign;
    if (!callB4b)    return ReplyPriorityCategory::NewCallsignBandMode;
    if (m_config.lotw_users().user(callsign)) return ReplyPriorityCategory::LoTWUser;
    return ReplyPriorityCategory::WorkedBefore;
}

ReplyPriorityCategory MainWindow::classifyReplyPriority(QString const& cqPriority) const
{
    if (cqPriority == "New DXCC")               return ReplyPriorityCategory::NewDXCC;
    if (cqPriority == "New DXCC on Band")       return ReplyPriorityCategory::NewDXCCBandMode;
    if (cqPriority == "New CQ Zone")            return ReplyPriorityCategory::NewCQZone;
    if (cqPriority == "New CQ Zone on Band")    return ReplyPriorityCategory::NewCQZoneBandMode;
    if (cqPriority == "New ITU Zone")           return ReplyPriorityCategory::NewITUZone;
    if (cqPriority == "New ITU Zone on Band")   return ReplyPriorityCategory::NewITUZoneBandMode;
    if (cqPriority == "New Grid")               return ReplyPriorityCategory::NewGrid;
    if (cqPriority == "New Grid on Band")       return ReplyPriorityCategory::NewGridBandMode;
    if (cqPriority == "New Continent")          return ReplyPriorityCategory::NewContinent;
    if (cqPriority == "New Continent on Band")  return ReplyPriorityCategory::NewContinentBandMode;
    if (cqPriority == "New Call")               return ReplyPriorityCategory::NewCallsign;
    if (cqPriority == "New Call on Band")       return ReplyPriorityCategory::NewCallsignBandMode;
    if (cqPriority == "LoTW User")              return ReplyPriorityCategory::LoTWUser;
    return ReplyPriorityCategory::WorkedBefore;
}

bool MainWindow::isBetterCandidate(AutoSeqCandidate const& a, AutoSeqCandidate const& b) const
{
    if (static_cast<int>(a.category) < static_cast<int>(b.category)) return true;
    if (static_cast<int>(a.category) > static_cast<int>(b.category)) return false;
    if (a.is_lotw && !b.is_lotw) return true;
    if (!a.is_lotw && b.is_lotw) return false;
    if (ui->respondComboBox->currentText() == "CQ: Max Dist") {
        if (a.distance_km > b.distance_km) return true;
        if (a.distance_km < b.distance_km) return false;
    } else {
        if (a.snr > b.snr) return true;
        if (a.snr < b.snr) return false;
    }
    return a.decode_order < b.decode_order;
}

AutoSeqCandidate MainWindow::bestAutoSeqCandidate() const
{
    if (m_autoSeqCandidates.isEmpty()) return AutoSeqCandidate{};
    AutoSeqCandidate best = m_autoSeqCandidates[0];
    for (int i = 1; i < m_autoSeqCandidates.size(); ++i) {
        if (isBetterCandidate(m_autoSeqCandidates[i], best))
            best = m_autoSeqCandidates[i];
    }
    return best;
}


void MainWindow::writeSyncDiagLog(double avg, int used, float spread, bool valid, bool candidate, bool updateAccepted, double lag)
{
  QFile file(m_syncDiagLogPath);
  ...
  if (!file_exists || !m_syncDiagHeaderWritten) {
    out << "UTC,Band,Mode,Avg,Used,Spread,Valid,Streak,Candidate,UpdateAccepted,Offset,OffsetDelta,Lag,Decodes,SyncEnabled\n";
    m_syncDiagHeaderWritten = true;
  }

  QString utc = QDateTime::currentDateTimeUtc().toString("yyyy-MM-dd HH:mm:ss");
  ...
  out << utc << ","
      << band << ","
      << mode << ","
      << QString::number(avg, 'f', 3) << ","
      << used << ","
      << QString::number(spread, 'f', 3) << ","
      << (valid ? 1 : 0) << ","
      << m_avgValidStreak << ","
      << (candidate ? 1 : 0) << ","
      << (updateAccepted ? 1 : 0) << ","
      << QString::number(m_dtOffset, 'f', 3) << ","
      << QString::number(m_lastOffsetDelta, 'f', 3) << ","
      << QString::number(lag, 'f', 3) << ","
      << m_nDecodes << ","
      << (m_useDtOffset ? 1 : 0)
      << "\n";
}
